...
# Jitsi Meet - Extension Installer
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BOLD}🧩 Instalador de Extensión: Jitsi Meet${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
TARGET_JITSI_DIR="${TARGET_EXTENSIONS_DIR}/jitsi_meet"
...
if [ -n "${SCRIPT_DIR}" ] && [ -f "${SCRIPT_DIR}/install.py" ]; then
    ...
    rm -rf "${TARGET_JITSI_DIR}"
    cp -r "${SCRIPT_DIR}" "${TARGET_JITSI_DIR}"
else
    ...
    if git clone --depth 1 "${EXT_REPO_URL}" "${TMP_DIR}"; then
        rm -rf "${TARGET_JITSI_DIR}"
        cp -r "${TMP_DIR}/jitsi_meet" "${TARGET_JITSI_DIR}"
        rm -rf "${TMP_DIR}"
    else
...
if [ -z "${OPTION}" ] && [ -t 0 ]; then
    echo ""
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${GREEN}1)${NC} 📹 Instalar Jitsi Meet Extension"
...
case "${OPTION}" in
    1|"")
        echo -e "\n${GREEN}🚀 Instalando extensión Jitsi Meet...${NC}"
        (cd "${KOGNITO_DIR}" && PYTHONPATH=. "${PYTHON}" "${TARGET_JITSI_DIR}/install.py")
        ;;
    2)
        echo -e "\n${YELLOW}🔄 Desinstalando extensión Jitsi Meet...${NC}"
        (cd "${KOGNITO_DIR}" && PYTHONPATH=. "${PYTHON}" "${TARGET_JITSI_DIR}/install.py" --uninstall)
