...
# KogniMail Email Management - Extension Installer
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BOLD}🧩 Instalador de Extensión: KogniMail / Email Management${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
TARGET_EMAIL_DIR="${TARGET_EXTENSIONS_DIR}/email_management"
...
if [ -n "${SCRIPT_DIR}" ] && [ -f "${SCRIPT_DIR}/install.py" ]; then
    ...
    rm -rf "${TARGET_EMAIL_DIR}"
    cp -r "${SCRIPT_DIR}" "${TARGET_EMAIL_DIR}"
else
    ...
    if git clone --depth 1 "${EXT_REPO_URL}" "${TMP_DIR}"; then
        rm -rf "${TARGET_EMAIL_DIR}"
        cp -r "${TMP_DIR}/email_management" "${TARGET_EMAIL_DIR}"
        rm -rf "${TMP_DIR}"
    else
...
if [ -z "${OPTION}" ]; then
    echo ""
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${GREEN}1)${NC} 📧 Instalar KogniMail Email Management"
...
case "${OPTION}" in
    1)
        echo -e "\n${GREEN}🚀 Instalando extensión KogniMail...${NC}"
        (cd "${KOGNITO_DIR}" && PYTHONPATH=. "${PYTHON}" "${TARGET_EMAIL_DIR}/install.py")
        ;;
    2)
        echo -e "\n${YELLOW}🔄 Desinstalando extensión KogniMail...${NC}"
        (cd "${KOGNITO_DIR}" && PYTHONPATH=. "${PYTHON}" "${TARGET_EMAIL_DIR}/install.py" --uninstall)
